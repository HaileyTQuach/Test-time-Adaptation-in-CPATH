...
def find_bns_(parent, prior):
    ...
    for name, child in parent.named_children():
        if isinstance(child, nn.BatchNorm2d):
            ...
            replace_mods.extend(find_bns_(child, prior))
    return replace_mods
...
class TBR(nn.Module):
    ...
    def forward(self, input):
        ...
        out = input * self.layer.weight[None,:,None,None] + self.layer.bias[None,:,None,None]
...
class DELTA(nn.Module):
    ...
    def reset(self):
        ...
        if self.args.norm_type=='rn':
            replace_mods = find_bns_(self.model, self.args.old_prior)
